...
export const content = defineProse({
  ...
  <>
    ...
    >
      <StatementMain>
        <P>
          <B>Любое</B> уравнение, состоящее из набора множителей (скобок), произведение которых равно нулю, можно
          решить, если по отдельности <B accent>приравнять к нулю каждый из множителей</B>.
        </P>
        <Diagram>{math`
        flowchart TD
      ...
      <StatementProof>
        <P>
          Мы обнаружили способ находить корни, но для <Dep on={equationSolution}>"решения уравнения"</Dep> надо не
          только найти корни, но и доказать, что <B>нет других корней</B>, помимо уже найденных.
        </P>
        <P>
          Давайте предположим, что существует какой-то пропущенный нами корень <M>x'</M>, который невозможно обнаружить
          по методу нулевых множителей. По определению, подстановка этого корня вместо неизвестной в уравнение должна
          дать верное равенство:
        </P>
        <BlockMath>{math`
          11x'(x'-8)(x'^2 - 4)\cdots = 0 \\
        `}</BlockMath>
        <P>
          В нашем случае "верное равенство" означает, что всё произведение слева станет равным нулю. А в каком случае
          умножение множителей может оказаться равным нулю? Только если один из этих множителей равен нулю! Других
          вариантов <Ref to={uniques._whyNoOtherOptions}>не существует</Ref>.
        </P>
        <Details $={uniques._whyNoOtherOptions} title="Почему других вариантов не существует?">
          <P>
            Представим, что в равенстве <M>a \cdot b = 0</M>, ни <M>a</M>, ни <M>b</M> нулю не равны. Поделим обе части
            равенства на <M>a</M>. Получится <M>b = 0 / a</M>. Ноль при делении на любое число (кроме ноля) даёт ноль.
            Значит <M>b = 0</M>. Получаем противоречие -- <M>b</M> одновременно равно нулю и не равно нулю. Выходит, что
            наше предположение о том, что оба числа <M>a</M> и <M>b</M> не равны нулю, неверно -- в таких произведениях
            хотя бы один из множителей <B>обязан</B> быть равным нулю.
          </P>
          <QED />
        </Details>
        <P>
          Раз при подстановке корень <M>x'</M> обратил в ноль какой-то из множителей, то мы его обязаны были обнаружить
          заранее. Получили взаимоисключающие утверждения, бессмыслицу -- корень <M>x'</M> был пропущен нами и при этом
          никак не мог быть пропущен. Значит, наше исходное предположение о существовании "лишнего" корня <M>x'</M>{' '}
          неверно, никаких лишних корней нет, и приравниванием к нулю множителей мы нашли <B>все</B> возможные корни
          уравнения.
        </P>
        <QED />
      </StatementProof>
