...
const quadraticPolynomial = $CONTENT.foundations.equations.quadratic.whatIsIt.article.$quadraticPolynomial;
...
export default defineProse({
  ...
  <>
    ...
    <P>
      ...
      раза подряд вынести общий множитель за скобки, можно совершить тёмную магию:
    </P>
    ...
    <P>
      У нас была сумма из трёх слагаемых (<M>x^2</M>, <M>8x</M> и <M>15</M>), а получилось произведение из двух
    ...
    >
      ...
      <Li>
        <P>
          <B>Способ решать квадратные уравнения</B>
        </P>
        <P>
    ...
    <P>
      ...
      числа, назовём их <M>t</M> и <M>k</M>. Причём разложить надо не как попало (таких разложений бесконечно много), а
    ...
    <Problems $={uniques.manualFactoringExamples} title="Примеры ручного разложения на множители" level="example">
      ...
      <SubProblem>
        ...
        <ProblemHint>
          ...
          отрицательными! Ведь любую разность можно всё равно записать в виде суммы: <M>2 - 3 = 2 + (-3)</M> или даже{' '}
          <M>-8 - 10 = (-8) + (-10)</M>!
        </ProblemHint>
        <ProblemAnswer>
      ...
      <SubProblem>
        ...
        <ProblemHint>Воспользуйтесь формулой сокращённого умножения.</ProblemHint>
        <ProblemAnswer>
          <BlockMath>(x-1)(x+1)</BlockMath>
        </ProblemAnswer>
        <ProblemSolution>
          <P>
            Это выражение можно разложить на множители, используя формулу сокращённого умножения под названием "разность
            квадратов":
          </P>
          <BlockMath>{math`
    ...
    <P>
      ...
      сокращённого умножения "разность квадратов", чтобы получить финальные множители разложения. В упрощенном виде
    ...
    <P>
      ...
      выносим его за скобки из всех трёх слагаемых:
    </P>
    ...
    <P>
      Выведенная формула записи любого квадратного трёхчлена через множители позволяет получить очень замечательный и
      интересный факт про квадратные трёхчлены и квадратные уравнения:
    </P>

    <Important
      $={uniques.quadraticFromAny}
      title="Любые числа это корни!"
      snippet={{
        key: true,
        seo: 'Любая пара чисел является корнями квадратного трёхчлена',
        description: `
          Из общей формулы разложения на множители следует, что любые два числа являются корнями какого-то квадратного трёхчлена.
        `,
      }}
    >
      <P>
        Сам факт существования формулы <M>A(x-x_1)(x-x_2)</M> означает, что{' '}
        <B accent>для любых двух чисел существует бесконечное количество квадратных трёхчленов</B>, корнями которых эти
        числа и являются!
      </P>
      <P>
        Возьмем числа <M>5</M> и <M>-2</M>. Подставляем их вместо <M>x_1</M> и <M>x_2</M> в формулу разложения на
        множители и, выбирая любые значения для <M>A</M>, можно получать сколько угодно квадратных трёхчленов:
      </P>
      <BlockMath>{math`
    ...
    <H1
      ...
      snippet={{
        key: true,
        seo: 'Координатная плоскость корней квадратных уравнений',
        title: 'Плоскость корней',
        description: `
          Геометрическая визуализация того факта, что абсолютно любые два числа являются корнями какого-то квадратного трёхчлена.
        `,
      }}
    >
      Плоскость корней квадратных уравнений
    </H1>

    <P>
      Классный факт того, что из любых двух чисел <Dep on={uniques.quadraticFromAny}>образуется</Dep> квадратный
      трёхчлен можно не менее классным образом визуализировать. Пару чисел (корней) можно изобразить в виде точки на
      координатной плоскости, где одна из осей отвечает за "первые" корни, а вторая -- за "вторые" корни. Любая точка на
      этой плоскости будет обозначать какой-то квадратный трёхчлен. Назовём эту визуализацию "плоскостью корней
      квадратных трёхчленов". Как вам? 😎
    </P>

    <Image src={rootsPlane} invert="dark" width="550px" />

    <P>
      Коэффициент <M>A</M> можно не учитывать, потому что он никак не способен повлиять на корни, а лишь кратно
      увеличивает или уменьшает получившиеся коэффициенты:
    </P>
    ...
    <List ordered>
      <Li>
        ...
        <P>
          ...
          прямоугольника или линии на плоскости корней.
        </P>
      </Li>
      <Li>
        <P>
